...
| Concurrency | Implemented in latest `origin/main`: coarse/dense scan windows are concurrent, different bilingual events are concurrent, same-event English-to-Chinese order is preserved. Needs smoke validation before strong pitch claims. | 最新 `origin/main` 已实现并发：粗扫/dense 窗口并发，不同双语事件并发，同一事件保持英文到中文顺序。强宣称前需要 smoke 验证。 |
| Rate-limit handling | Runner has request staggering and retry/backoff knobs. | runner 已有请求错峰和 retry/backoff 参数。 |
...
- Run a small concurrent job with low `--concurrency`, then raise it only if logs are clean.
...
- 先用低 `--concurrency` 跑小并发任务，日志干净后再提高并发。
